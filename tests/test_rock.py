...
# ---- head-hit reaction (animation cue, separate from the stun) ----

def test_head_reaction_fires_on_every_head_connect_floored_to_light():
    # even a fresh jab to the head snaps it -> at least a light reaction
    assert damage.head_reaction("head_center", "clean", 0.5) == "light"
    assert damage.head_reaction("head_center", "glancing", 0.5) == "light"


def test_head_reaction_tiers_up_by_health_damage():
    assert damage.head_reaction("head_center", "clean", _R["head_dmg"]["medium"]) == "medium"
    assert damage.head_reaction("head_center", "clean", _R["head_dmg"]["hard"]) == "hard"


def test_head_reaction_none_for_body_or_blocked():
    assert damage.head_reaction("body_left", "clean", 99.0) is None     # no head animation for a body shot
    assert damage.head_reaction("head_center", "blocked", 99.0) is None  # guard caught it -> no head snap


def test_land_event_carries_reaction_for_head_shot():
    red, blue = _pair()
    blue.left.state = blue.right.state = FREE
    rec = _rec()
    _impact(red, blue, _landed_hand("jab", "head_center", 3), 1.0, make_rng(1), Fight(), rec)
    land = [e for e in rec._events if e["kind"] == "land"][-1]
    assert land["reaction"] == "light"   # a head jab doesn't rock, but still plays a light head reaction
    assert land["rock"] == "none"
